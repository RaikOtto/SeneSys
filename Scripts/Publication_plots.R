...
expr = expr_raw[row_var > quantiles[95],]
dim(expr)
...
#meta_data$PFS[is.na(meta_data$PFS)] = 0
#meta_data$PFS_vis = meta_data$PFS
#meta_data$PFS_vis[meta_data$PFS_vis == 0] = "No_progression"
#meta_data$PFS_vis[meta_data$PFS_vis != "No_progression"] = "Progression"
...
p = pheatmap::pheatmap(
  expr,
  #correlation_matrix,
  annotation_col = meta_data[,c("Drup_response",selection)],
...
umap_p = ggplot(
  umap_result$layout,
  aes(x, y))
#umap_p = umap_p + geom_point( aes( size = 4, color = as.character(meta_data$ABC_GCB) ))
umap_p = umap_p+geom_text(size= 2,aes(label=rownames(meta_data),color = as.character(meta_data$ABC_GCB)),hjust=0, vjust=0)
#umap_p = umap_p + theme(legend.position = "none") + xlab("") + ylab("")
umap_p = umap_p + stat_ellipse( linetype = 1, aes( color = meta_data$ABC_GCB), level=.5, type ="t", size=1.5)
umap_p = umap_p + scale_color_manual( values = c("darkgreen","yellow","darkred")) ##33ACFF ##FF4C33
umap_p
